...
volatile byte Sys11LastLowerSol = 0x00;
...
byte RPU_GetSolenoidCount() {
  // fifteen pulsed, and 4 continuous solenoids
  return 15 + 4;
}

boolean RPU_GetSolenoidStatus(byte solNumber) {
  boolean solOn = false;
  if (solNumber>18) return false;

  if (solNumber>14) {
    // the continuous solenoids are mapped to 15 - 18
    byte solBitMask = 0x10 << (solNumber-15);
    if ((CurrentSolenoidByte & solBitMask)) solOn = false;
    else solOn = true;
  } else {
    byte solNumOn = CurrentSolenoidByte & 0x0F;
    if (solNumOn==SOL_NONE) {
      solOn = false;
    } else {
      if (solNumOn==solNumber) solOn = true;
      else solOn = false;
    }
  }
